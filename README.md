...
![](files/shard_data.png)

Таким образом расположение данных по шардам примет следующий вид:

![](files/database_masters.jpg)
...
Поскольку мы нацелены на Российскую аудиторию, а сервера S3 находятся за рубежом, то мы установим кэширующее прокси в России,
что уменьшит время ожидания для популярных объявлений.
...
Таким образом схема проекта примет следующий вид:
...
Пример расчета для микросевиса объявлений: с учетом того, что одно ядро на бэкенде может выдержать до 100 rps то, 
чтобы выдержать заявленную нагрузку (max. 27 тыс. rps) при закупке 32-х ядерных машин нам потребуется [27000/(32*100)] = 9 таких машин,
для обеспечения отказоустойчивости и с учетом ежегодного прироста пользователей поставим 15 таких машин.
...
Поскольку наша аудитория в первую очередь это Россия, то наши сервера будут раскиданы внутри страны по разным дата центам MRG, которые предоставляет MCS (Москва-Север, Москва-Восток).
...
В k8s кластере при развертывании новой фермы на неё вешается отдельный dns адрес, который позволяет осуществлять балансировку по подам.
Подробнее об этом можно прочитать [тут](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service).

Алгоритм работы кэширующих прокси: для обеспечения высокого попадания в кэш запрос за одной и той же фотографией всегда должен приходить на один и тот же прокси. 
Будем выбирать прокси по хэшу запрашиваемого url, для обеспечения нормального горизонтального масштабирования будем использовать [consistent hashing](https://en.wikipedia.org/wiki/Consistent_hashing).
...
Отказоустойчивость всех наших микросервисов на бэкенде, балансировщиков и кэширующих прокси обеспечивается за счет их развертки в k8s, 
